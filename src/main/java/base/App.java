...
The program should display the subtotal, tax, and total for Wisconsin residents but display
just the total for non-residents.
...
public class App {
    public static void main(String[] args) {
        ...
        double totalPrice = orderAmount;

        if(stateName.equals("WI")){

            taxComment = String.format("The tax is $%.2f.\n",tax/100*orderAmount);
            subTotalComment= String.format("The subtotal is $%.2f.\n", orderAmount);
            totalPrice += (orderAmount*(tax/100));

        }
        String totalComment = String.format("The total is $%.2f.", totalPrice);
